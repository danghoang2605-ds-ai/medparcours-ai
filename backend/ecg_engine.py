...
def _detrend_signal(arr: np.ndarray) -> np.ndarray:
    """
    Khử trôi dạt baseline (vd do ảnh chụp nghiêng làm đường isoelectric dốc
    dần theo chiều ngang) bằng rolling median với cửa sổ rộng hơn 1 phức bộ
    QRS nhưng hẹp hơn 1 chu kỳ tim — giữ lại biến động NHANH (QRS), loại bỏ
    xu hướng CHẬM (trôi dạt do nghiêng).

    PHÁT HIỆN QUA TEST CÓ HỆ THỐNG (không phải ca demo cụ thể): ảnh nghiêng
    từ 5 độ trở lên làm height_threshold tuyệt đối trong detect_r_peaks() bị
    đánh lừa bởi baseline trôi dần, gây đếm trùng đỉnh giả và nhịp tim sai
    tăng vọt (vd 75bpm thật bị tính ra 360bpm) — TRONG KHI calibration vẫn
    báo "độ tin cậy cao", không có cảnh báo nào cho người dùng. Đây chính là
    loại lỗi nguy hiểm nhất: tự tin báo sai. Detrend xử lý đúng gốc nguyên
    nhân (baseline trôi), không chỉ thêm cảnh báo che giấu vấn đề.

    CHỈ áp dụng cho việc TÌM ĐỈNH R, KHÔNG áp dụng cho signal[] trả về ở
    digitize_ecg_image() — vì xu hướng trôi dạt có thể là tín hiệu lâm sàng
    thật (ST chênh) cần giữ nguyên cho bác sĩ xem, không nên tự ý xóa.
    """
    n = len(arr)
    if n < 9:
        return arr  # quá ngắn để detrend có ý nghĩa
    window = min(71, n - 1 if (n - 1) % 2 == 1 else n - 2)
    if window < 5:
        return arr
    if window % 2 == 0:
        window -= 1
    ...
    detrended = arr - trend
    d_min, d_max = detrended.min(), detrended.max()
    if d_max - d_min < 1e-9:
        return arr  # không có biến động gì để chuẩn hóa -> trả nguyên signal gốc
    return (detrended - d_min) / (d_max - d_min)
...
def detect_r_peaks(signal: list, height_threshold: float = 0.5,
    ...
    BƯỚC 0 (detrend): khử trôi dạt baseline trước khi tìm đỉnh — xem
    _detrend_signal() để biết lý do và bằng chứng phát hiện lỗi.
    ...
    arr = _detrend_signal(np.array(signal, dtype=np.float64))
