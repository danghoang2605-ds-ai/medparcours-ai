...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .upload-recent{margin-top:20px;padding-top:16px;border-top:1px solid var(--border);display:flex;flex-direction:column;align-items:center;gap:8px}
  .upload-attention{margin-top:20px;padding:14px 16px;background:#FFFBEB;border:1px solid #FDE68A;border-radius:14px;display:flex;flex-direction:column;align-items:center;gap:8px}
  ...
  .upload-recent-chips{display:flex;flex-wrap:wrap;gap:7px;justify-content:center}
          ...
          <div className="nav-row1">
            ...
            <div className="nav-right">
              ...
              {pkey === "x" ? (
                ...
                {savedStatus === "da_luu" && (
                  <button className="nav-save-btn primary saved" onClick={()=>setUpdatePanelOpen(true)} title={`Cập nhật hồ sơ${savedMeta ? ` (đã lưu, lần ${savedMeta.so_lan_cap_nhat})` : ""}`} aria-label="Cập nhật hồ sơ">
...
function renderBookmarkDetail(it) {
  ...
  return it.detail
    ? it.detail
    : "Chưa lưu nội dung chi tiết cho mục này (đánh dấu từ bản cũ). Hãy bỏ đánh dấu rồi đánh dấu lại để lưu kèm nội dung."
}
  ...
  return (
    <div className="reason-item" style={{ background:sv.bg, borderColor:sv.border }}>
      <div className="reason-head" onClick={()=>setOpen(o=>!o)}>
        ...
        <FeedbackBtn pkey={CURRENT_PKEY} muc="Biện luận lâm sàng" contentSnippet={`${item.tieu_de}: ${item.noi_dung}`}/>
                ...
                <div className="lab-spark-dates">
                  {(() => {
                    ...
                    if (phaseInfo && phaseInfo.surg) {
                      ...
                      // 4 mốc lâm sàng, rơi về đầu/giữa/cuối như trước.
                      idxs = n <= 3 ? m.trendDates.map((_,i)=>i) : [0, Math.round((n-1)/2), n-1]
                    }
                    return idxs.map(di => {
...
function ReportTab({ report: r, analysis }) {
  ...
  return (
    <div className="report-stack">
      ...
      <IcdGroupsCard groups={activeIcdGroups} pkey={r.thong_tin_benh_nhan && r.thong_tin_benh_nhan.so_benh_an}/>

      <MedDiagChangesPanel report={r} pkey={r.thong_tin_benh_nhan && r.thong_tin_benh_nhan.so_benh_an}/>
...
.hist-pin-btn.pinned{color:#D97706;background:#FFFBEB;border-color:#FDE68A}
...
.cw-panel{width:340px;max-width:calc(100vw - 40px);height:480px;max-height:calc(100vh - 120px);background:var(--glass);border:1px solid var(--border);border-radius:16px;box-shadow:0 10px 25px rgba(0,0,0,.1);display:flex;flex-direction:column;overflow:hidden;transform-origin:bottom right;animation:cw-panel-in .18s ease-out}
...
.cw-close{position:absolute;top:9px;right:9px;flex-shrink:0;width:22px;height:22px;border-radius:50%;border:none;background:rgba(100,116,139,.1);color:#64748B;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:background .15s}
...
function UnifiedChatWidget({ report, hoSoText, clinicalMessages, setClinicalMessages }) {
  ...
  const sendClinical = async (text) => {
    ...
    try {
      const res = await fetch(`${API_URL}/chat`, { method:"POST", headers:{ "Content-Type":"application/json" },
        body:JSON.stringify({ question:q, ho_so_text:hoSoText||JSON.stringify(report), chat_history:clinicalMessages.slice(-6), mode:"clinical" }) })
      const data = await res.json()
      if (!res.ok || !data || !data.answer) throw new Error(data?.detail || "no answer")
  ...
  return (
    <div className="faq-widget-root">
      {(open || closing) && (
        <div className={`cw-panel${closing ? " closing" : ""}`}>
      ...
      <button className="faq-fab" onClick={()=>open ? closePanel() : setOpen(true)} title="MedAmi — trợ lý lâm sàng & hỗ trợ hệ thống">
        {open ? <Icon.Close d={20} color="#fff"/> : <Icon.Chat d={20} color="#fff"/>}
        {!open && <span className="faq-fab-lbl">MedAmi</span>}
