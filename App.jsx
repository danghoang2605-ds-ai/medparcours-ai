...
const mpApi = {
  ...
  ekycOcrCccd: async (fileFront) => {
    ...
    const res = await fetch(`${API_URL}/ekyc/ocr-cccd`, { method: "POST", body: fd })
  ...
  ekycFaceLiveness: async (fileFace) => {
    ...
    const res = await fetch(`${API_URL}/ekyc/face-liveness`, { method: "POST", body: fd })
    const data = await res.json()
  ...
  summarizeConsultationAudio: async (audioFile) => {
    ...
    const res = await fetch(`${API_URL}/consultation/summarize-audio`, { method: "POST", body: fd })
    const data = await res.json()
...
function CoStat({ m }) {
  ...
  if (tr.length > 1) {
    ...
    // Đệm thêm 15% biên độ trên/dưới — dữ liệu dao động nhỏ (vd CRP đi từ 80
    // xuống 75, chênh lệch nhỏ so với tổng thang giá trị) trước đây co giãn
    // hết cỡ trục Y khiến đường trông "phẳng dài rồi rơi dốc đứng ở cuối"
    // (đã xác nhận qua ảnh chụp thực tế) — thêm đệm giúp đường mềm mại hơn.
    const rawRng = (mx - mn) || 1
    const pad = rawRng * 0.15
    const mn2 = mn - pad, rng = rawRng + pad * 2
    const pts = tr.map((v, i) => [P + i * (W - 2*P) / (tr.length - 1), H - P - ((v - mn2) / rng) * (H - 2*P)])
    // Làm mượt bằng đường cong Catmull-Rom xấp xỉ (qua điểm điều khiển giữa
    // 2 điểm liên tiếp) thay vì nối thẳng góc cạnh — mượt mắt hơn hẳn với
    // dữ liệu ít điểm mà vẫn giữ đúng hình dạng xu hướng thật.
    if (pts.length === 2) {
      path = `M${pts[0][0].toFixed(1)} ${pts[0][1].toFixed(1)} L${pts[1][0].toFixed(1)} ${pts[1][1].toFixed(1)}`
    } else {
      path = `M${pts[0][0].toFixed(1)} ${pts[0][1].toFixed(1)} `
      for (let i = 0; i < pts.length - 1; i++) {
        const p0 = pts[i], p1 = pts[i+1]
        const mx2 = (p0[0] + p1[0]) / 2
        path += `Q${p0[0].toFixed(1)} ${p0[1].toFixed(1)} ${mx2.toFixed(1)} ${((p0[1]+p1[1])/2).toFixed(1)} `
      }
      path += `T${pts[pts.length-1][0].toFixed(1)} ${pts[pts.length-1][1].toFixed(1)}`
    }
  ...
  return (
    <div className="chat-wrap">
      <div className="chat-mode-toggle">
        ...
        <button className={`chat-mode-btn${isFaq?" active":""}`} onClick={()=>setChatMode("faq")}>FAQ Bệnh viện</button>
      </div>
