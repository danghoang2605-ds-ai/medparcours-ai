...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .prio-tools-light .card-tool-btn{color:rgba(255,255,255,0.65)}
  .prio-tools-light .card-tool-btn:hover{background:rgba(255,255,255,0.15);color:#fff}
  .prio-tools-light .card-tool-btn.has-content{background:rgba(255,255,255,0.2);color:#fff}
  ...
  /* UPLOAD */
  .upload-page{min-height:100vh;position:relative;overflow-x:hidden;background:var(--page-bg)}
  ...
  .hero-wrap{position:relative;z-index:10;max-width:1280px;margin:0 auto;padding:52px 32px 32px;display:grid;grid-template-columns:1fr 1fr;gap:48px;align-items:center}
  @media(max-width:900px){.hero-wrap{grid-template-columns:1fr;padding:32px 20px 24px;gap:28px}}
  ...
  .upload-merged{padding:36px 26px;min-height:340px;display:flex;align-items:center;justify-content:center}
  .upload-merged-inner{width:100%;text-align:center}
  .upload-merged-actions{display:flex;gap:12px;justify-content:center;margin:18px 0 20px;flex-wrap:wrap}
  .upload-merged-actions .btn-primary{flex:1;min-width:170px;justify-content:center}
  @media(max-width:480px){.upload-merged-actions{flex-direction:column}.upload-merged-actions .btn-primary{min-width:0}}
  ...
  .report-nav-inner{max-width:1120px;margin:0 auto;padding:10px 24px 8px;display:flex;flex-direction:column;gap:9px}
  .nav-row1{display:flex;align-items:center;justify-content:space-between;gap:10px}
  .nav-row2{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}
  .nav-left{display:flex;align-items:center;gap:14px}
  .nav-sep{color:rgba(200,220,255,0.5);font-size:10px}
  .nav-patient{display:flex;align-items:center;gap:8px;min-width:0}
  ...
  .patient-name{font-size:13.5px;font-weight:600;color:var(--navy2);white-space:nowrap}
  .patient-meta{font-size:11px;background:rgba(29,111,232,0.08);color:var(--navy3);padding:3px 9px;border-radius:6px;white-space:nowrap;flex-shrink:0}
...
// ─── Ghi âm giọng nói (Web Speech API vi-VN) — TÁCH RIÊNG từ AudioRecorder
// để tái dùng cho ghi chú per-widget (mỗi widget có nút ghi âm riêng trong
// panel "Ghi chú của bác sĩ"), không chỉ dùng cho "lời dặn tổng" ở UploadPage.
function useSpeechToText(onTranscript) {
  ...
  const start = async (currentValue) => {
    setErr("")
    ...
    r.onresult = (e) => {
      ...
      onTranscript && onTranscript(finalRef.current + interim)
    }
    r.onerror = (e) => {
...
function Card({ id, title, icon, children, headRight, defaultCollapsed = false }) {
  ...
  const tools = useWidgetTools(widgetId)
  ...
  return (
    <div id={id} className={`card${collapsed ? " collapsed" : ""}`}>
      ...
      <div className="card-body">
        <WidgetEditableBody tools={tools} bodyRef={bodyRef}>{children}</WidgetEditableBody>
      </div>
...
function ReportPage({ report, hoSoText, analysis, onReset, onReportUpdated, chatMessages, setChatMessages, onOpenHistory, onLogout }) {
  ...
  const goToBookmark = (it) => {
    ...
    setViewMode(mode); setTab("report")
              ...
              <div className="nav-menu-wrap">
                ...
                {menuOpen && <>
                  ...
                  <div className="nav-menu">
                    ...
                    <button onClick={()=>{setMenuOpen(false);toggleFocusMode()}}>
                      {focusMode
                        ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14h6v6M20 10h-6V4M14 10l7-7M3 21l7-7"/></svg>
                        : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>}
                    {focusMode ? "Tắt chế độ trình chiếu" : "Chế độ trình chiếu"}
                  </button>
                  <button onClick={()=>{setMenuOpen(false);setTab("bookmarks")}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg>Mục đã đánh dấu{bmList.length>0?` (${bmList.length})`:""}</button>
                  <button onClick={()=>{setMenuOpen(false);mpCards(true);mpToast("Đã thu gọn tất cả thẻ")}}><Icon.ChevUp d={14} color="#475569"/>Thu gọn tất cả thẻ</button>
                  <button onClick={()=>{setMenuOpen(false);mpCards(false);mpToast("Đã mở rộng tất cả thẻ")}}><Icon.ChevDown d={14} color="#475569"/>Mở rộng tất cả thẻ</button>
...
function TrajectoryCard({ assessment }) {
  ...
  const bodyRef = useRef(null)
  const tools = useWidgetTools("sec-trajectory")
  ...
  return (
    <div id="sec-trajectory" className="traj-card" style={{ background:tm.bg, borderColor:tm.border }}>
      <div className="traj-head">
        ...
        <span style={{marginLeft:"auto",display:"inline-flex",gap:"6px",alignItems:"center"}}><WidgetToolButtons tools={tools}/><CopyBtn text={()=>evidence.map(e=>e.txt).join("\n")} label=""/></span><button className="banner-collapse dark" onClick={()=>setCollapsed(c=>!c)} title={collapsed?"Mở":"Thu gọn"} style={{ marginLeft:"8px" }}>
...
function SummaryCard({ text }) {
  const [collapsed, setCollapsed] = useGlobalCollapse(false)
  const bodyRef = useRef(null)
  const tools = useWidgetTools("sec-summary")
  ...
  return (
    <div id="sec-summary" className="summary-card">
      <div className="summary-hd">
        ...
        <span style={{marginLeft:"auto",display:"inline-flex",gap:"6px",alignItems:"center"}}><WidgetToolButtons tools={tools}/></span>
...
function PhaseSection({ phase, events, info, ketLuan }) {
  const [collapsed, setCollapsed] = useGlobalCollapse(false)
  const bodyRef = useRef(null)
  const tools = useWidgetTools("sec-phase" + phase)
...
function ProblemStatus({ data, pkey }) {
  const [collapsed, setCollapsed] = useGlobalCollapse(false)
  const bodyRef = useRef(null)
  const tools = useWidgetTools("sec-problems", pkey)
  if (!data) return null
  return (
    <div id="sec-problems" className="ov-card">
      <div className="ov-head">
        <Icon.Octagon d={16} color="#1D6FE8"/><span>Trạng thái vấn đề lâm sàng</span>
        <span style={{marginLeft:"auto",display:"inline-flex",gap:"6px",alignItems:"center"}}><WidgetToolButtons tools={tools}/><FlagBtn pkey={CURRENT_PKEY} label="Trạng thái vấn đề lâm sàng" sub="Mục báo cáo" detail={()=>((data.hien_tai)||[]).map(p=>`${p.ten}: ${p.mo_ta||""}`).join("\n")}/><CopyBtn text={()=>((data.hien_tai)||[]).map(p=>`${p.ten}: ${p.mo_ta||""}`).join("\n")} label=""/></span>
        <button className="banner-collapse dark" onClick={()=>setCollapsed(c=>!c)} title={collapsed?"Mở":"Thu gọn"} style={{ marginLeft:"6px" }}>
          {collapsed ? <Icon.ChevDown d={14} color="#1D6FE8"/> : <Icon.ChevUp d={14} color="#1D6FE8"/>}
        </button>
      </div>
...
// ─── Hook chung cho MỌI widget (Card chuẩn HOẶC widget tự code riêng) ────────
// Gộp state Chỉnh sửa (sửa trực tiếp nội dung hiển thị, contentEditable) +
// Ghi chú (block mở rộng riêng, có ghi âm, đóng lại khi rỗng) — dùng 1 hook
// để mọi nơi trong App.jsx có hành vi NHẤT QUÁN, không viết lại từng chỗ.
function useWidgetTools(widgetId, pkey) {
  ...
  const [noteText, setNoteText] = useState(() => noteGet(key, widgetId))
  const savedNote = noteGet(key, widgetId)
  const openNote = () => { setNoteText(noteGet(key, widgetId)); setNoteOpen(o => !o) }
  const closeNoteIfEmpty = () => {
    const had = !!savedNote
    noteSet(key, widgetId, noteText)
    if (!noteText.trim()) { setNoteOpen(false); if (had) mpToast("Đã xóa ghi chú") }
  }
  const saveNote = () => {
    const had = !!savedNote
    noteSet(key, widgetId, noteText)
    if (!noteText.trim()) { if (had) mpToast("Đã xóa ghi chú") }
    else mpToast("Đã lưu ghi chú")
  }
...
// ─── Nút Chỉnh sửa + Ghi chú dùng chung — gắn vào header của BẤT KỲ widget ───
// (Card chuẩn hoặc banner tự code riêng). Đặt cạnh các nút có sẵn (cờ/copy).
function WidgetToolButtons({ tools }) {
  return (
    <>
      <button className={`card-tool-btn${tools.savedEdit ? " has-content" : ""}`} onClick={tools.startEdit} title="Chỉnh sửa trực tiếp" aria-label="Chỉnh sửa">
        <Icon.Pencil d={13} color={tools.savedEdit ? "#1D6FE8" : "currentColor"}/>
      </button>
      <button className={`card-tool-btn${tools.savedNote ? " has-content" : ""}`} onClick={tools.openNote} title="Ghi chú của bác sĩ" aria-label="Ghi chú">
        <Icon.Note d={13} color={tools.savedNote ? "#D97706" : "currentColor"}/>
      </button>
    </>
  )
}

// Biến thể CHỈ có nút Ghi chú, KHÔNG có Chỉnh sửa trực tiếp — dùng cho widget
// có dữ liệu cấu trúc phức tạp (timeline nhiều cấp...) mà sửa trực tiếp bằng
// contentEditable dễ vỡ HTML khi áp lại bản đã sửa.
function WidgetNoteOnlyButton({ tools }) {
  return (
    <button className={`card-tool-btn${tools.savedNote ? " has-content" : ""}`} onClick={tools.openNote} title="Ghi chú của bác sĩ" aria-label="Ghi chú">
      <Icon.Note d={13} color={tools.savedNote ? "#D97706" : "currentColor"}/>
    </button>
...
// ─── Panel "Ghi chú của bác sĩ" — block mở rộng riêng (KHÔNG phải input 1
// dòng), có tiêu đề + nút ghi âm, đóng lại hoàn toàn nếu rỗng khi đóng ────────
function WidgetNotePanel({ tools }) {
  const stt = useSpeechToText(tools.setNoteText)
  if (!tools.noteOpen) return null
  return (
    <div className="wnote-panel">
      <div className="wnote-head">
        <Icon.Note d={14} color="#D97706"/><span>Ghi chú của bác sĩ</span>
        <button className="wnote-close" onClick={() => { tools.closeNoteIfEmpty(); if (stt.rec) stt.stop() }} title="Đóng" aria-label="Đóng"><Icon.Close d={13} color="#92400E"/></button>
      </div>
      <textarea className="wnote-ta" autoFocus value={tools.noteText} onChange={e => tools.setNoteText(e.target.value)}
        placeholder="Nhập ghi chú riêng cho mục này, hoặc bấm micro để đọc..."/>
      <div className="wnote-bar">
        <button type="button" className={`wnote-mic${stt.rec ? " on" : ""}`} onClick={() => stt.rec ? stt.stop() : stt.start(tools.noteText)} disabled={!stt.supported} title={stt.rec ? "Dừng ghi" : "Ghi âm bằng giọng nói"}>
          {stt.rec ? <><span className="rec-dot pulse"/>Đang nghe...</> : <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>Ghi âm</>}
        </button>
        <span style={{ flex: 1 }}/>
        <button type="button" className="wnote-save" onClick={() => { tools.saveNote(); if (stt.rec) stt.stop() }}>Lưu</button>
      </div>
      {stt.err && <div className="rec-note err"><Icon.Alert d={12} color="#B91C1C"/>{stt.err}</div>}
    </div>
  )
}
  ...
  return (
    <div className="bm-page">
      <div className="bm-page-head">
        <button className="bm-back" onClick={onBack} title="Quay lại báo cáo"><Icon.Back d={16} color="#1D6FE8"/>Quay lại báo cáo</button>
        <div className="bm-page-title"><span className="bm-head-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/></svg></span>
      ...
      <div className="bm-page-body">
        {(!items || items.length===0) && (
          <div className="bm-empty">
            <span className="bm-empty-icon"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg></span>
...
function ClinicalTakeaway({ items, pkey }) {
  const [collapsed, setCollapsed] = useGlobalCollapse(false)
  const bodyRef = useRef(null)
  const tools = useWidgetTools("sec-takeaway", pkey)
  ...
  return (
    <div id="sec-takeaway" className="takeaway-card">
      <div className="takeaway-hd"><Icon.Stethoscope d={15} color="#1D6FE8"/><span>Kết luận lâm sàng nhanh</span>
        <span style={{marginLeft:"auto",display:"inline-flex",gap:"6px",alignItems:"center"}}><WidgetToolButtons tools={tools}/><FlagBtn pkey={CURRENT_PKEY} label="Kết luận lâm sàng nhanh" sub="Mục báo cáo" detail={()=>items.map(i=>i.txt).join("\n")}/><CopyBtn text={()=>items.map(i=>i.txt).join("\n")} label=""/></span>
        <button className="banner-collapse dark" onClick={()=>setCollapsed(c=>!c)} title={collapsed?"Mở":"Thu gọn"} style={{ marginLeft:"6px" }}>
          {collapsed ? <Icon.ChevDown d={14} color="#1D6FE8"/> : <Icon.ChevUp d={14} color="#1D6FE8"/>}
        </button>
      </div>
              ...
              {m.trendDates && m.trendDates.length >= 2 && m.trend && m.trend.length === m.trendDates.length && (
                <div className="lab-spark-dates">
                  {(() => {
                    ...
                    // Giới hạn tối đa 3 nhãn (đầu/giữa/cuối) khi có nhiều điểm đo —
                    // trước đây hiện HẾT mọi ngày, với chỉ số đo ≥4-5 lần các nhãn
                    // chồng lên nhau đọc không nổi (vd Na+/K+/Cl-/Glucose đo hàng ngày).
                    const idxs = n <= 3 ? m.trendDates.map((_,i)=>i) : [0, Math.round((n-1)/2), n-1]
                    return idxs.map(di => {
...
function PriorityBanner({ findings, onSource, pkey }) {
  ...
  const bodyRef = useRef(null)
  const tools = useWidgetTools("sec-priority", pkey)
    ...
    <div id="sec-priority" className="prio-wrap">
      <div className="prio-head">
        ...
        <div className="prio-head-r">
          ...
          <span className="prio-tools-light"><WidgetToolButtons tools={tools}/></span>
...
function TimelineStrip({ events }){
  ...
  const tools = useWidgetTools("sec-timeline")
...
function DoctorNote({ value, onChange }){
  ...
  const stt = useSpeechToText(onChange)
    ...
    <>
      ...
      {open && (
        <div className="dn-panel">
          <div className="dn-head">{pencil}<span>Ghi chú của bác sĩ</span><button className="dn-x" onClick={()=>{setOpen(false); if(stt.rec) stt.stop()}} aria-label="Đóng"><Icon.Close d={13} color="#64748B"/></button></div>
          ...
          <div className="dn-bar">
            <button type="button" className={`dn-mic${stt.rec?" on":""}`} onClick={()=>stt.rec?stt.stop():stt.start(value)} disabled={!stt.supported} title={stt.rec?"Dừng ghi":"Ghi âm bằng giọng nói"}>
              {stt.rec ? <><span className="rec-dot pulse"/>Đang nghe...</> : <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>Ghi âm</>}
            </button>
            <span className="dn-foot">{(value||"").trim().length} ký tự · tự lưu</span>
          </div>
          {stt.err && <div className="rec-note err"><Icon.Alert d={12} color="#B91C1C"/>{stt.err}</div>}
        </div>
      )}
...
function SpecCard({ y }){
  ...
  const bodyRef = useRef(null)
  const tools = useWidgetTools("mdt-" + (y.khoa||"spec"))
  return (
    <div className="spec-card">
      <div className="spec-hd">
        ...
        <span style={{marginLeft:"auto",display:"inline-flex",gap:"4px"}}><WidgetToolButtons tools={tools}/></span>
      </div>
      <WidgetEditableBody tools={tools} bodyRef={bodyRef}>
...
function TeachCard({ n, title, children, accent }){
  const [collapsed, setCollapsed] = useGlobalCollapse(false)
  const bodyRef = useRef(null)
  const tools = useWidgetTools("teach-" + n)
  return (
...
const ECG_DEMO_SAMPLES = {"dongA_1":{"ten":"Nguyễn Văn A — ECG ngày 25/11/2025 (62 tuổi)","is_real_patient_scan":true,"doc_diem_lam_sang":{"thong_so_ky_thuat":{"toc_do_ghi":"25 mm/s (ghi rõ trên ảnh gốc)","bien_do_chuan":"10 mm/mV (ghi rõ trên ảnh gốc)","so_chuyen_dao":"Trích 1 chuyển đạo (Lead I) từ bản ghi 12 chuyển đạo đầy đủ gốc — hệ thống hiện CHƯA tự tách được nhiều chuyển đạo cùng lúc từ 1 ảnh trang đầy đủ (xem 'han_che_ky_thuat')."},"nhip":{"loai":"Nhịp xoang (Sinus rhythm)","ghi_chu":"Theo kết luận tự động của máy đo — bản gốc ghi \"Unconfirmed Report\" (chưa có bác sĩ ký xác nhận)."},"tan_so":{"gia_tri":83,"don_vi":"lần/phút","phan_loai":"Bình thường (60-100 lần/phút)","nguon":"Số máy đo ghi trên ảnh gốc (Vent. rate: 83 bpm) — đáng tin hơn số ước tính tự động của thuật toán hiện tại, xem 'han_che_ky_thuat'."},"truc":{"gia_tri":"P/QRS/T = 27/-16/270°","ly_do":"Trục theo máy đo ghi trên ảnh gốc (bản đầy đủ 12 chuyển đạo) — PR 178ms, QRS 96ms, QT/QTc 358/398ms. Hệ thống MedParcours hiện CHƯA tự tính được trục điện tim từ ảnh (cần ≥2 chuyển đạo ở 2 mặt phẳng khác nhau), số liệu này lấy trực tiếp từ máy đo gốc."},"nghi_ngo":{"tinh_trang":"Bất thường sóng T (nghi thiếu máu cơ tim thành dưới), có nhiễu khi đo","ly_giai":"Bất thường sóng T, nghi thiếu máu cơ tim thành dưới (T wave abnormality, possible inferior ischemia) · Có nhiễu tín hiệu khi đo (ARTIFACT PRESENT)","nhan":"Hỗ trợ quan sát — đây là ECG THẬT của bệnh nhân, kết luận trên là do MÁY ĐO tự động sinh ra (chưa qua bác sĩ xác nhận), KHÔNG phải kết luận của hệ thống MedParcours. Cần bác sĩ đọc lại trên bản ghi 12 chuyển đạo đầy đủ."}},"image_base64":"data:image/jpeg;base64,","signal":[0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.9427,0.
,0.3336,0.3946,0.4286,0.412,0.3246,0.1932,0.082,0.0399,0.0415,0.0522,0.0558,0.0575,0.0576,0.0577,0.0577,0.0578,0.0579,0.058,0.0581,0.0582,0.0583,0.0584,0.0585,0.0585,0.0586,0.0587,0.0588,0.0589,0.059,0.0591,0.0592,0.0592,0.0593,0.0594,0.0595,0.0596,0.0597,0.0598,0.0599,0.0599,0.06,0.0601,0.0602,0.0603,0.0604,0.0605,0.0606,0.0605,0.0598,0.0599,0.0628,0.0671,0.0704,0.0726,0.0723,0.0696,0.0654,0.0617,0.0614,0.0641,0.0675,0.0698,0.0549,0.0506,0.1018,0.2188,0.3496,0.4369,0.4627,0.4195,0.31,0.1682,0.0611,0.0322,0.0405,0.0481,0.0515,0.0522,0.052,0.0518,0.0517,0.0515,0.0513,0.0511,0.0509,0.0508,0.0506,0.0504,0.0502,0.05,0.0494,0.0492,0.0499,0.0513,0.0531,0.0549,0.0565,0.0575,0.0576,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0571,0.0564,0.0564,0.0561,0.0589,0.065,0.0733,0.084,0.0969],"width":455,"height":65,"columns_with_signal":455,"che_do_phat_hien":"grayscale","calibration":{"px_per_mm":4.0,"do_tin_cay":"trung_binh","warning":"Ảnh có chiều cao thấp (65px) — dải đo lưới ở mép trên chỉ 7px, không đủ để trung bình hóa nhiễu ảnh. Tỉ lệ px/mm và nhịp tim suy ra có thể kém chính xác hơn ảnh đầy đủ 12 chuyển đạo, cần đối chiếu cẩn thận với số máy đo gốc."},"r_peaks":{"peaks":[77,143,204,273,331,404],"rr_intervals_px":[66,61,69,58,73],"warning":null},"heart_rate":{"bpm_avg":92.4,"bpm_per_beat":[90.9,98.4,87.0,103.4,82.2],"rr_seconds":[0.66,0.61,0.69,0.58,0.73],"rr_range_seconds":0.15,"nhip_deu_theo_nguong_sach":true,"nguong_ap_dung":"Chênh lệch R-R lớn nhất/nhỏ nhất < 0.16s (mượn ngưỡng PP của nhịp xoang đều theo sách lý thuyết — CHƯA xác nhận bởi Tấn/Ngân cho mục đích R-R)","uoc_luong":true,"warning":null,"_note":"Số bpm THẬT theo máy đo ghi trên ảnh gốc là 83 bpm — đáng tin hơn số ước tính tự động (92.4 bpm) của thuật toán số hóa hiện tại. Chênh lệch do thuật toán mới được test trên ảnh trang 12 chuyển đạo thật lần đầu, còn đang hiệu chỉnh."},"han_che_ky_thuat":"Hệ thống hiện chỉ số hóa được ĐÚNG 1 dải chuyển đạo đã cắt sẵn (Lead I), CHƯA tự động tách được nhiều chuyển đạo từ 1 ảnh trang đầy đủ 12 chuyển đạo — nếu tải nguyên ảnh trang đầy đủ, kết quả ước tính nhịp tim sẽ SAI nghiêm trọng do thuật toán nhầm lẫn giữa các dải chuyển đạo chồng nhau. Đây là hạn chế đã biết, đang tìm hướng khắc phục.","disclaimer":"Kết quả số hóa và ước tính nhịp tim chỉ mang tính trực quan hóa hỗ trợ, cần bác sĩ xác nhận. Không phải kết luận chẩn đoán. Đây là ECG THẬT (đã ẩn danh) dùng để kiểm thử hệ thống, kết luận lâm sàng là do máy đo gốc tự sinh ra, chưa qua bác sĩ xác nhận."},"dongA_2":{"ten":"Nguyễn Văn A — ECG ngày 13/03/2026 (63 tuổi)","is_real_patient_scan":true,"doc_diem_lam_sang":{"thong_so_ky_thuat":{"toc_do_ghi":"25 mm/s (ghi rõ trên ảnh gốc)","bien_do_chuan":"10 mm/mV (ghi rõ trên ảnh gốc)","so_chuyen_dao":"Trích 1 chuyển đạo (Lead I) từ bản ghi 12 chuyển đạo đầy đủ gốc — hệ thống hiện CHƯA tự tách được nhiều chuyển đạo cùng lúc từ 1 ảnh trang đầy đủ (xem 'han_che_ky_thuat')."},"nhip":{"loai":"Nhịp xoang (Sinus rhythm)","ghi_chu":"Theo kết luận tự động của máy đo — bản gốc ghi \"Unconfirmed Report\" (chưa có bác sĩ ký xác nhận)."},"tan_so":{"gia_tri":71,"don_vi":"lần/phút","phan_loai":"Bình thường (60-100 lần/phút)","nguon":"Số máy đo ghi trên ảnh gốc (Vent. rate: 71 bpm) — đáng tin hơn số ước tính tự động của thuật toán hiện tại, xem 'han_che_ky_thuat'."},"truc":{"gia_tri":"P/QRS/T = 36/-21/-30°","ly_do":"Trục theo máy đo ghi trên ảnh gốc (bản đầy đủ 12 chuyển đạo) — PR 208ms, QRS 100ms, QT/QTc 382/404ms. Hệ thống MedParcours hiện CHƯA tự tính được trục điện tim từ ảnh (cần ≥2 chuyển đạo ở 2 mặt phẳng khác nhau), số liệu này lấy trực tiếp từ máy đo gốc."},"nghi_ngo":{"tinh_trang":"Bất thường sóng T (nghi thiếu máu cơ tim thành bên), lệch trục trái vừa","ly_giai":"Bất thường sóng T, nghi thiếu máu cơ tim thành bên (T wave abnormality, possible lateral ischemia) · Lệch trục trái mức độ vừa (Moderate left axis deviation) · Có nhiễu tín hiệu khi đo (ARTIFACT PRESENT)","nhan":"Hỗ trợ quan sát — đây là ECG THẬT của bệnh nhân, kết luận trên là do MÁY ĐO tự động sinh ra (chưa qua bác sĩ xác nhận), KHÔNG phải kết luận của hệ thống MedParcours. Cần bác sĩ đọc lại trên bản ghi 12 chuyển đạo đầy đủ."}},"image_base64":"data:image/jpeg;base64,","signal":[0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3883,0.3873,0.3925,0.3965,0.3917,0.3805,0.3575,0.3198,0.2672,0.2092,0.1585,0.1166,0.0816,0.0586,0.0359,0.0,0.0319,0.2286,0.5199,0.7583,0.8647,0.8337,0.6681,0.3933,0.1369,0.0137,0.0193,0.0581,0.0679,0.0686,0.0701,0.0708,0.0709,0.0717,0.0731,0.0771,0.0857,0.098,0.1105,0.1227,0.1325,0.1413,0.1475,0.1493,0.1498,0.149,0.1423,0.1335,0.1245,0.1173,0.1114,0.1055,0.1008,0.1008,0.1037,0.1089,0.1168,0.1285,0.1409,0.1533,0.1648,0.1751,0.1844,0.1887,0.1856,0.1796,0.1781,0.1787,0.178,0.1766,0.1758,0.1746,0.1717,0.1659,0.1631,0.1706,0.183,0.2004,0.2342,0.2805,0.3268,0.3624,0.3807,0.3785,0.3558,0.3112,0.2586,0.2137,0.1746,0.1437,0.1137,0.0618,0.0395,0.1553,0.432,0
,0.063,0.0637,0.0651,0.069,0.0713,0.074,0.08,0.0903,0.1033,0.118,0.1331,0.1477,0.16,0.1682,0.1694,0.1668,0.1614,0.1519,0.1384,0.1208],"width":455,"height":65,"columns_with_signal":455,"che_do_phat_hien":"grayscale","calibration":{"px_per_mm":4.0,"do_tin_cay":"trung_binh","warning":"Ảnh có chiều cao thấp (65px) — dải đo lưới ở mép trên chỉ 7px, không đủ để trung bình hóa nhiễu ảnh. Tỉ lệ px/mm và nhịp tim suy ra có thể kém chính xác hơn ảnh đầy đủ 12 chuyển đạo, cần đối chiếu cẩn thận với số máy đo gốc."},"r_peaks":{"peaks":[120,196,272,349,426],"rr_intervals_px":[76,76,77,77],"warning":null},"heart_rate":{"bpm_avg":78.4,"bpm_per_beat":[78.9,78.9,77.9,77.9],"rr_seconds":[0.76,0.76,0.77,0.77],"rr_range_seconds":0.01,"nhip_deu_theo_nguong_sach":true,"nguong_ap_dung":"Chênh lệch R-R lớn nhất/nhỏ nhất < 0.16s (mượn ngưỡng PP của nhịp xoang đều theo sách lý thuyết — CHƯA xác nhận bởi Tấn/Ngân cho mục đích R-R)","uoc_luong":true,"warning":null,"_note":"Số bpm THẬT theo máy đo ghi trên ảnh gốc là 71 bpm — đáng tin hơn số ước tính tự động (78.4 bpm) của thuật toán số hóa hiện tại. Chênh lệch do thuật toán mới được test trên ảnh trang 12 chuyển đạo thật lần đầu, còn đang hiệu chỉnh."},"han_che_ky_thuat":"Hệ thống hiện chỉ số hóa được ĐÚNG 1 dải chuyển đạo đã cắt sẵn (Lead I), CHƯA tự động tách được nhiều chuyển đạo từ 1 ảnh trang đầy đủ 12 chuyển đạo — nếu tải nguyên ảnh trang đầy đủ, kết quả ước tính nhịp tim sẽ SAI nghiêm trọng do thuật toán nhầm lẫn giữa các dải chuyển đạo chồng nhau. Đây là hạn chế đã biết, đang tìm hướng khắc phục.","disclaimer":"Kết quả số hóa và ước tính nhịp tim chỉ mang tính trực quan hóa hỗ trợ, cần bác sĩ xác nhận. Không phải kết luận chẩn đoán. Đây là ECG THẬT (đã ẩn danh) dùng để kiểm thử hệ thống, kết luận lâm sàng là do máy đo gốc tự sinh ra, chưa qua bác sĩ xác nhận."},"dongA_3":{"ten":"Nguyễn Văn A — ECG ngày 09/05/2026 (63 tuổi)","is_real_patient_scan":true,"doc_diem_lam_sang":{"thong_so_ky_thuat":{"toc_do_ghi":"25 mm/s (ghi rõ trên ảnh gốc)","bien_do_chuan":"10 mm/mV (ghi rõ trên ảnh gốc)","so_chuyen_dao":"Trích 1 chuyển đạo (Lead I) từ bản ghi 12 chuyển đạo đầy đủ gốc — hệ thống hiện CHƯA tự tách được nhiều chuyển đạo cùng lúc từ 1 ảnh trang đầy đủ (xem 'han_che_ky_thuat')."},"nhip":{"loai":"Nhịp xoang (Sinus rhythm)","ghi_chu":"Theo kết luận tự động của máy đo — bản gốc ghi \"Unconfirmed Report\" (chưa có bác sĩ ký xác nhận)."},"tan_so":{"gia_tri":67,"don_vi":"lần/phút","phan_loai":"Bình thường (60-100 lần/phút)","nguon":"Số máy đo ghi trên ảnh gốc (Vent. rate: 67 bpm) — đáng tin hơn số ước tính tự động của thuật toán hiện tại, xem 'han_che_ky_thuat'."},"truc":{"gia_tri":"P/QRS/T = 45/-26/-43°","ly_do":"Trục theo máy đo ghi trên ảnh gốc (bản đầy đủ 12 chuyển đạo) — PR 224ms, QRS 110ms, QT/QTc 368/383ms. Hệ thống MedParcours hiện CHƯA tự tính được trục điện tim từ ảnh (cần ≥2 chuyển đạo ở 2 mặt phẳng khác nhau), số liệu này lấy trực tiếp từ máy đo gốc."},"nghi_ngo":{"tinh_trang":"Bloc nhĩ-thất độ 1, ST chênh xuống/lên bất thường, lệch trục trái vừa — CÓ điện cực bị tuột khi đo","ly_giai":"Bloc nhĩ-thất độ 1 (First degree AV block) — PR kéo dài 224ms · ST chênh xuống mức độ vừa (Moderate ST depression) · ST chênh lên — nghi viêm màng ngoài tim/tổn thương dưới thượng tâm mạc/tái cực sớm (ST elevation, consistent with subepicardial injury, pericarditis, or early repolarization) · Bất thường ST-T không đặc hiệu (Nonspecific ST & T abnormality) · Lệch trục trái mức độ vừa (Moderate left axis deviation) · ĐIỆN CỰC BỊ TUỘT khi đo — máy yêu cầu đo lại (ELECTRODE(S) DETACHED ... Repeat ECG is requested)","nhan":"Hỗ trợ quan sát — đây là ECG THẬT của bệnh nhân, kết luận trên là do MÁY ĐO tự động sinh ra (chưa qua bác sĩ xác nhận), KHÔNG phải kết luận của hệ thống MedParcours. Cần bác sĩ đọc lại trên bản ghi 12 chuyển đạo đầy đủ."}},"image_base64":"data:image/jpeg;base64,","signal":[0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5909,0.5915,0.5908,0.5894,0.5876,0.5869,0.591,0.5964,0.6017,0.6065,0.6079,0.603,0.5965,0.593,0.5969,0.6086,0.6244,0.6417,0.6549,0.6606,0.6583,0.6497,0.6375,0.6254,0.6162,0.6091,0.6034,0.6004,0.6008,0.6048,0.6105,0.6196,0.6312,0.6442,0.658,0.6727,0.6848,0.6938,0.701,0.7065,0.7092,0.71,0.7138,0.7241,0.7377,0.7518,0.7679,0.7861,0.8022,0.8158,0.8276,0.8388,0.8504,0.866,0.8837,0.9034,0.9292,0.9592,0.9872,1.0,0.988,0.9476,0.8818,0.799,0.7077,0.6117,0.5167,0.433,0.3518,0.2465,0.1726,0.1811,0.2473,0.2944,0.3051,0.2881,0.235,0.132,0.0366,0.0,0.01,0.0288,0.0291,0.0295,0.0299,0.0302,0.0306,0.0309,0.0313,0.0317,0.032,0.0324,0.0327,0.0331,0.0335,0.0338,0.0342,0.0345,0.0349,0.0353,0.0356,0.036,0.0364,0.0367,0.0371,0.0374,0.0378,0.0382,0.0385,0.0389,0.0392,0.0396,0.04,0.0403,0.0407,0.041,0.0397,0.0376,0.0384,0.0497,0.0709,0.0987,0.1279,0.155,0.1764,0.1904,0.1979,0.2052,0.2126,0.2182,0.22,0.2185,0.2166,0.2199,0.2306,0.2487,0.275,0.3074,0.339,0.3662,0.3847,0.39,0.382,0.3632,0.3394,0.3136,0.291,0.2756,0.2646,0.2365,0.2158,
h đầy đủ 12 chuyển đạo, cần đối chiếu cẩn thận với số máy đo gốc."},"r_peaks":{"peaks":[82,178,419],"rr_intervals_px":[96,241],"warning":null},"heart_rate":{"bpm_avg":43.7,"bpm_per_beat":[62.5,24.9],"rr_seconds":[0.96,2.41],"rr_range_seconds":1.45,"nhip_deu_theo_nguong_sach":false,"nguong_ap_dung":"Chênh lệch R-R lớn nhất/nhỏ nhất < 0.16s (mượn ngưỡng PP của nhịp xoang đều theo sách lý thuyết — CHƯA xác nhận bởi Tấn/Ngân cho mục đích R-R)","uoc_luong":true,"warning":null,"_note":"Số bpm THẬT theo máy đo ghi trên ảnh gốc là 67 bpm — đáng tin hơn số ước tính tự động (43.7 bpm) của thuật toán số hóa hiện tại. Chênh lệch do thuật toán mới được test trên ảnh trang 12 chuyển đạo thật lần đầu, còn đang hiệu chỉnh."},"han_che_ky_thuat":"Hệ thống hiện chỉ số hóa được ĐÚNG 1 dải chuyển đạo đã cắt sẵn (Lead I), CHƯA tự động tách được nhiều chuyển đạo từ 1 ảnh trang đầy đủ 12 chuyển đạo — nếu tải nguyên ảnh trang đầy đủ, kết quả ước tính nhịp tim sẽ SAI nghiêm trọng do thuật toán nhầm lẫn giữa các dải chuyển đạo chồng nhau. Đây là hạn chế đã biết, đang tìm hướng khắc phục.","disclaimer":"Kết quả số hóa và ước tính nhịp tim chỉ mang tính trực quan hóa hỗ trợ, cần bác sĩ xác nhận. Không phải kết luận chẩn đoán. Đây là ECG THẬT (đã ẩn danh) dùng để kiểm thử hệ thống, kết luận lâm sàng là do máy đo gốc tự sinh ra, chưa qua bác sĩ xác nhận."}};
...
// phải có chẩn đoán đi kèm"): trước đây panel chỉ hiện 2 ô số (bpm + độ tin cậy
// px/mm) — đúng về kỹ thuật nhưng vô nghĩa với bác sĩ đọc điện tim. Form mới
// bắt buộc xuất đủ 5 mục theo cách đọc ECG chuẩn:
//   0. Thông số kỹ thuật ghi (tốc độ, biên độ chuẩn, số chuyển đạo)
//   1. Nhịp (xoang/không xoang, cụ thể loại gì)
//   2. Tần số
//   3. Trục điện tim (trung gian/trái/phải/vô định)
//   4. Nghi ngờ tình trạng + lý giải
...
function EcgReadingForm({ result, dd }) {
  ...
  return (
    <div className="ecg-reading-form">
      ...
      <EcgFormRow icon="0" label="Thông số kỹ thuật ghi" statusChip={dd ? "Có ghi chú" : "Chưa xác định"} statusColor={dd ? coXacDinh : chuaXacDinh}>
        {dd ? (
          <ul className="ecg-form-list">
            ...
            <li><b>Biên độ chuẩn:</b> {dd.thong_so_ky_thuat.bien_do_chuan}</li>
      ...
      {hr?.nhip_deu_theo_nguong_sach === false && (
      ...
      {hr?.warning && !dd && (
        <div className="rec-note err"><Icon.Alert d={13} color="#B91C1C"/>{hr.warning}</div>
      )}
      {!dd && hr?.bpm_avg != null && (
        <div className="rec-note warn">
          <Icon.Alert d={13} color="#92400e"/>
          Hạn chế kỹ thuật hiện tại: số tần số tự ước tính (thuật toán Mức 2) còn sai lệch nhiều trên ảnh nhiễu, chưa hiệu chỉnh đầy đủ theo thông số kỹ thuật ghi thật của ảnh — đang cải thiện.
        </div>
      )}
...
function EcgPage({ onBack, initialResult, onLogout }) {
  ...
  // Demo tĩnh: 2 ảnh ECG THẬT (đã xin phép dùng làm minh họa), kết quả số hóa
  // đã tính sẵn bằng đúng ecg_engine.py (không gọi API, không bịa số). Phần
  // bpm hiển thị lấy theo SỐ MÁY ĐO GHI TRÊN ẢNH GỐC (đáng tin hơn số tự ước
  // lượng của thuật toán hiện tại — xem ghi chú "_note" trong từng mẫu) — vì
  // mục đích demo là cho người xem thấy ĐÚNG giao diện/luồng hoạt động, còn
  // độ chính xác thuật toán Mức 2 vẫn đang cải thiện dần (xem nguon_guideline
  // và warning nếu Đăng muốn xem số tự tính thực tế thay vì số máy đo).
  const loadDemo = (key) => {
    const sample = ECG_DEMO_SAMPLES[key]
    if (!sample) return
    setError(null)
    setStaged({ url: sample.image_base64, name: sample.ten })
    setResult(sample)
    setIsDemo(true)
  }
      ...
      <div className="ecg-page-body">
        ...
        {!staged && (
          <>
            ...
            <div className="ecg-demo-row">
              <span className="ecg-demo-lbl">Hoặc xem demo với ECG thật của Nguyễn Văn A (đã ẩn danh, 3 lần đo theo dõi diễn tiến):</span>
        ...
        {staged && (
          <div className="ecg-workspace-v2">
            ...
            {result && (
              <EcgReadingForm result={result} dd={result.doc_diem_lam_sang}/>
            )}
          </div>
        )}
      </div>
...
.dn-bar{display:flex;align-items:center;gap:9px;padding:8px 14px;border-top:1px solid var(--border);background:#FAFBFD}
.dn-mic{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--border);background:#fff;color:var(--navy2);border-radius:999px;padding:5px 12px;font-size:11.5px;font-weight:600;cursor:pointer}
.dn-mic.on{background:#DC2626;color:#fff;border-color:#DC2626}
.dn-mic:disabled{opacity:.5;cursor:not-allowed}
.dn-foot{font-size:11px;color:var(--muted)}
...
body.theme-dark .dn-bar{background:#10192A;border-color:#28364E}
body.theme-dark .dn-foot{color:#A8BBD6}
body.theme-dark .dn-mic{background:#1B2536;border-color:#2F4368;color:#9FB3CC}
...
.bm-page{max-width:900px;margin:0 auto;padding:24px 28px 60px;min-height:100vh;box-sizing:border-box}
.bm-back{display:inline-flex;align-items:center;gap:7px;border:none;background:transparent;color:#1D6FE8;font-size:13.5px;font-weight:600;cursor:pointer;padding:8px 0;margin-bottom:14px}
.bm-back:hover{text-decoration:underline}
.bm-page-head{display:flex;align-items:center;flex-wrap:wrap;gap:10px;padding-bottom:16px;border-bottom:1px solid var(--border);margin-bottom:18px;width:100%}
.bm-page-title{display:flex;align-items:center;gap:9px;font-size:19px;font-weight:700;color:var(--navy);flex:1}
...
.bm-page-body{display:flex;flex-direction:column;gap:0}
body.theme-dark .bm-page-head{border-color:#28364E}
body.theme-dark .bm-page-title{color:#EAF1FB}
...
@media(max-width:860px){
  ...
  .report-nav-inner{padding:8px 12px 6px}
  .nav-row2{gap:6px 8px}
...
.ecg-demo-btn:hover{background:#DCEAFD}
.ecg-workspace{display:grid;grid-template-columns:1fr 1fr;gap:20px}
.ecg-workspace-v2{display:flex;flex-direction:column;gap:18px}
