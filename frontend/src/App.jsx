...
function computeEGFR(creatUmol, age, sexMale) {
  // ĐỒNG BỘ với compute_egfr() trong clinical_rules.py — đã sửa lỗi age=0
  // (sơ sinh) bị "!age" coi là falsy nên luôn trả null sai, và chặn creatinin
  // không hợp lệ (<=0) tránh NaN/số phức khi tính mũ.
  if (creatUmol == null || age == null) return null
  if (creatUmol <= 0 || age < 0) return null
...
// Cụm "cảnh báo nguy cơ dự phòng" — ĐỒNG BỘ với PROSPECTIVE_RISK_PHRASES
// trong clinical_rules.py (xem ghi chú đầy đủ ở đó về lý do thêm nhóm này).
const CRS_PROSPECTIVE_RISK_PHRASES = ["nguy co","can phong ngua","can theo doi","co kha nang","trong tuong lai","de phong","canh bao nguy co"]
...
function crsHasAnyPositive(haystack, keywords) {
  for (const kw of keywords) {
    ...
    while (true) {
      ...
      const isNegated = CRS_NEGATION_PHRASES.some(neg => window.includes(neg))
      const isProspective = CRS_PROSPECTIVE_RISK_PHRASES.some(p => window.includes(p))
      if (!isNegated && !isProspective) return true
      start = idx + kw.length
    }
  }
  return false
